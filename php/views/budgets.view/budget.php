...
<div class="main1">
    ...
    <?php if (isset($_SESSION['budget_Create_Error'])) {
    $errors = $_SESSION['budget_Create_Error'];
    ...
    unset($_SESSION['budget_Create_Error']);
}?>

    <?php if (isset($_SESSION['budget_Update_Error'])) {
    $errors = $_SESSION['budget_Update_Error'];
    ...
    unset($_SESSION['budget_Update_Error']);
}?>
    ...
    <script>
        ...
        async function updateInventory(id, currentCategory) {
            ...
            data.append("currentCategory", currentCategory);
            data.append("category", category);
            data.append("description", description);
            data.append("amount", amount);

            const response = await fetch('../../includes/budget.inc.php', {
                method: 'POST',
                body: data
            })
        ...
        async function showUpdateForm(sel) {
            ...
            const response = await fetch('budgetUpdateForm.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json; charset=utf-8'
                },
        ...
        function deleteBudgetData(sel) {
            let confirmation = confirm('Are you sure you want to delete this?');
            if(confirmation) {
            data = new FormData();
            data.append("delete", sel)

            fetch('../../includes/budget.inc.php', {
                method: 'POST',
                body: data
            }).then(response=>{
                showTableForm();
        ...
        async function showTableForm(page) {
            ...
            try {
            const response = await fetch('budgetTableUpdater.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json; charset=utf-8'
                },
