...
class salesController extends Sales
{
    ...
    public function setsalesAddData($itemName, $itemDescription, $quantity, $amount, $total)
    {
    ...
    public function setsalesUpdateData($currentItemName, $id, $itemNo, $itemName, $itemDescription, $quantity, $amount)
    {
        $this->id = $id;
        $this->itemNo = $itemNo;
        $this->itemName = $itemName;
    ...
    public function setsalesDeleteData($id)
    {
        $this->boss = $_SESSION['admin_name'];
        $this->id = $id;
    }
    ...
    public function addsales()
    {
        ...
        # ERROR HANDLER 7
        // if ($this->is_sales_category_budget_is_not_exist()) {
        //     $error["not_exist"] = "sales Budget Category does not Exist!";
        // }
        // # ERROR HANDLER 8
        // if ($this->is_sales_budget_not_good_enough()) {
        //     $error["over_budget"] = "Over Budget!";
        // }

        if ($error) {
            $_SESSION['sales_Add_Error'] = $error;
            header('Location: ../views/sales.view/sales.php'); //header ni kay form ni ya nd same sa update
            die();
        }
        ...
        // $this->decrease_sales_category_budget($finalDecreasedEmployeeBudgetData, $this->boss);
        #need mag dugang man sa expense mag bakal sales
        // $this->insertToExpensesales($this->boss, $this->currentDate, $this->Amount, $this->itemDescription, $this->amount);
        $this->createsales($this->itemNo, $this->boss, $this->currentDate, $this->itemName, $this->itemDescription, $this->quantity, $this->total, $this->amount);
    ...
    public function update_sales()
    {
        ...
        if ($error) {
            $_SESSION['sales_Update_Error'] = $error;

            echo 'sales.php';
            die();
        }
        $boss = $_SESSION["admin_name"];
        $this->updatesales($boss, $this->id, $this->itemNo, $this->itemName, $this->itemDescription, $this->quantity, $this->amount);
    }

    public function delete_sales() # done
    {
        $this->detele_id_sales($this->id, $this->boss);
    }

    # ERROR HANDLERS FOR ADD sales

    # 1
    private function is_inputs_empty() # EMPTY INPUTS NOT ALLOWED!
    { // done
        return (empty($this->itemName) or empty($this->itemDescription) or empty($this->quantity) or empty($this->amount)) ? true : false;
    }
    ...
    // }
    # 7
    private function is_sales_category_budget_is_not_exist()
    { // done
        return ($this->check_sales_category_budget_not_exist($this->boss)) ? true : false; # BOSS PARAMETER LNG KAY NA SPECIFY NAMAN TO NA sales NGA CATEGORY
    }
    # 8
    private function is_sales_budget_not_good_enough()
    { // done
        $budget = $this->get_sales_category_budget($this->boss);
    ...
    //////////////////for addsales and updatesales ERROR HANDLERS private functions///////////////////

    /* para sa mag check if the sales budget exist  kag budget amount{*/
